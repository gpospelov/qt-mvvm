...
void AxisObject::paint(QPainter* painter, const QStyleOptionGraphicsItem*, QWidget*)
{
    ...
    auto ticks_x = TicksView(
        fromSceneToRealX(fromAxisToSceneX(points[0][0])), 
        fromSceneToRealX(fromAxisToSceneX(points[1][0])));
    auto ticks_y = TicksView(
        fromSceneToRealY(fromAxisToSceneY(points[1][1])), 
    ...
    std::cout<<"point info"<<points[1][1]<<" "<<fromAxisToSceneY(points[1][1])<<" "<<points[2][1]<<" "<<fromAxisToSceneY(points[2][1])<<std::endl;
    std::cout<<"factor info"<<y_factor<<" "<<scene_rect.y()<<std::endl;
    std::cout<<"tick info "<<ticks_y.range<<" "<<ticks_y.minPoint<<" "<<ticks_y.maxPoint<<" "<<ticks_y.tickSpacing<<std::endl;

    double tick_length_x = 10;
    double tick_length_y = 10;

    // x ticks
    for (double i = ticks_x.niceMin + ticks_x.tickSpacing; i < ticks_x.niceMax; i+= ticks_x.tickSpacing){

        painter->drawLine(
            fromSceneToAxisX(fromRealToSceneX(i)), points[0][1]+pen_thickness/2.,
            fromSceneToAxisX(fromRealToSceneX(i)), points[0][1]+tick_length_y
        );
        painter->drawLine(
            fromSceneToAxisX(fromRealToSceneX(i)), points[2][1]-pen_thickness/2.,
        ...
        auto text_size = QFontMetrics(painter->font()).size(
                Qt::TextSingleLine, text
            );

        painter->drawText(
            fromSceneToAxisX(i) - text_size.width()/2.,
            points[0][1]+pen_thickness/2. - text_size.height()/2.,
            text
        );

        painter->drawText(
            fromSceneToAxisX(i) - text_size.width()/2.,
            points[2][1]+pen_thickness + text_size.height()/2.,
            text
        );
    }

    // y ticks
    for (float i = ticks_y.niceMin + ticks_y.tickSpacing; i < ticks_y.niceMax; i+= ticks_y.tickSpacing){

        painter->drawLine(
            points[0][0]+pen_thickness/2., fromSceneToAxisY(fromRealToSceneY(i)),
            points[0][0]+tick_length_y, fromSceneToAxisY(fromRealToSceneY(i))
        );

        painter->drawLine(
            points[1][0]-pen_thickness/2., fromSceneToAxisY(fromRealToSceneY(i)),
        ...
        painter->drawText(
            -(fromSceneToAxisY(fromRealToSceneY(i)) + text_size.width()/2.),
            points[0][0]+pen_thickness/2. - text_size.height()/2.,
            text
        );

        painter->drawText(
            -(fromSceneToAxisY(fromRealToSceneY(i)) + text_size.width()/2.),
            points[1][0]+pen_thickness + text_size.height()/2.,
            text
        );
