...
DesignerMimeData::DesignerMimeData(const std::string& entryname, const QString& xmldescr,
                                   QDrag* drag)
    ...
    if (reader.hasError()) {
        QString errorMessage = tr("A parse error occurred at line %1, column %2 of the XML code "
                                  "specified for the widget %3: %4\n%5")
                                   ...
                                   .arg(QString::fromStdString(m_entryname))
...
Qt::DropAction DesignerMimeData::execDrag(const std::string& name, const QString& xmldescr,
                                          QWidget* dragSource)
{
    if (!xmldescr.size())
