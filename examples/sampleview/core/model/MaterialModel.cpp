...
void MaterialModel::init_model()
{
    ...
    auto material = dynamic_cast<SLDMaterialItem*>(
        insertNewItem(::Constants::SLDMaterialType, container, MaterialContainerItem::T_MATERIALS));
    material->set_properties("Air", QColor(Qt::blue), 1e-06, 1e-07);

    material = dynamic_cast<SLDMaterialItem*>(
        insertNewItem(::Constants::SLDMaterialType, container, MaterialContainerItem::T_MATERIALS));
