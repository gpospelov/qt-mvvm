...
TEST_F(TestModelUtils, topItem)
{
    ...
    EXPECT_EQ(Utils::TopItem<ToyItems::MultiLayer>(&model), nullptr);
    ...
    EXPECT_EQ(Utils::TopItem<ToyItems::MultiLayer>(&model), multilayer1);
}
...
TEST_F(TestModelUtils, topItems)
{
    ...
    EXPECT_EQ(Utils::TopItems<ToyItems::MultiLayer>(&model).size(), 0);

    auto multilayer1 = dynamic_cast<ToyItems::MultiLayer*>(
        model.insertNewItem(ToyItems::Constants::MultiLayerType));
    auto particle = model.insertNewItem(ToyItems::Constants::ParticleType);
    auto multilayer2 = dynamic_cast<ToyItems::MultiLayer*>(
        model.insertNewItem(ToyItems::Constants::MultiLayerType));

    std::vector<SessionItem*> expected1 = {multilayer1, particle, multilayer2};
    EXPECT_EQ(Utils::TopItems<SessionItem>(&model), expected1);
    std::vector<ToyItems::MultiLayer*> expected2 = {multilayer1, multilayer2};
    EXPECT_EQ(Utils::TopItems<ToyItems::MultiLayer>(&model), expected2);
}
...
TEST_F(TestModelUtils, findItems)
{
    ...
    EXPECT_EQ(Utils::FindItems<ToyItems::MultiLayer>(&model).size(), 0);
    ...
    std::vector<ToyItems::MultiLayer*> expected2 = {multilayer1, multilayer2};
    EXPECT_EQ(Utils::FindItems<ToyItems::MultiLayer>(&model), expected2);

    // adding layers to multilayer
    auto layer1 = dynamic_cast<ToyItems::Layer*>(
        model.insertNewItem(ToyItems::Constants::LayerType, multilayer1));

    auto layer2 = dynamic_cast<ToyItems::Layer*>(
        model.insertNewItem(ToyItems::Constants::LayerType, multilayer2));

    std::vector<ToyItems::Layer*> expected3 = {layer1, layer2};
    EXPECT_EQ(Utils::FindItems<ToyItems::Layer>(&model), expected3);
}
